...
def strategy_2x2(const, players):
        for player in players:
            ...
            while not player.staged:
                ...
                strike_solve = puck_angle(const.opponent_player,player.me,const)
                strike_solve = puck_angle(const.opponent_player,player.me,const)
                ...
                strike_dev = abs((const.opponent_player.net_front-const.puck_x)*tan(base_angle)-
                                 (const.opponent_player.net_front-const.puck_x)*tan(base_angle+pi/180))
                ...
                if player.state == PlayerState.REST_FUNNY and \
                    ...
                    #print(player.me.id)
                    if player.me.id == nearest_teammate(const.game, const.world,const.def_x, const.def_y,False).id:
                ...
                if (player.state in PlayerState.PUCKMAN_STATES
                    ...
                    if const.puck_owner is not None and const.puck_owner.player_id != const.player.id:
                        if abs(const.puck_x - const.player.net_front) > const.rink_len/2:
                            ...
                            puck_nearest_opp = nearest_opponent(const.game, const.world, const.puck_owner.x, const.puck_owner.y,False)
                            if abs(puck_nearest_opp.x - const.player.net_front) < const.rink_len/2\
                                    and abs(const.puck_x - const.player.net_front) < const.rink_len/2:
                                player.state = PlayerState.DEFMAN_HELP_OWNER
                                player.target_unit = puck_nearest_opp
                            else:
                                player.state = PlayerState.DEFMAN_GODEF
                                player.target = PlayerTarget(const.def_x,const.def_y)
                    else:
                ...
                #pre-states for turn+swing
                    ...
                    if strike_solve_pass[2] < strike_dev:
                        player.state = PlayerState.PUCKMAN_GO_SIDE_POSITION
                        player.target = PlayerTarget(*get_position_point(player.me, const))
                    ...
                    if strike_solve_pass[2] >= strike_dev*2 and strike_solve_pass[2] > 2\
                            and abs(player.me.angle-get_angle(const.puck_x,const.puck_y,*get_net_position(player.me, const,strike_solve_pass,0))) <=const.game.pass_sector/2:
                        ...
                        if strike_window >= strike_dev*2:
                            if (nearest_me is None
                                or player.me.get_distance_to_unit(nearest_me) > const.game.stick_length*4)\
                                and strike_dev > 5:
                                chk_strike = 0
                    ...
                    if  window_top <= finish_puck_win_top <= finish_puck_win_bot <= window_bot\
                            and (True or nearest_me is None
                                or player.me.get_distance_to_unit(nearest_me) > const.game.stick_length
                                or nearest_me.remaining_cooldown_ticks > 1)\
                            and (True or nearest_puck is None
                                or const.world.puck.get_distance_to_unit(nearest_puck) > const.game.stick_length
                                or nearest_puck.remaining_cooldown_ticks > 1)\
                            and player.me.swing_ticks < max_swing\
                            :
                        player.state = PlayerState.PUCKMAN_SWING
                        if player.me.swing_ticks == 0:
                            print('NET STAT window: {0} goal: {1} bot: {2} target: {3}'.format(str(strike_solve[0]),
                                                                       str(window_top),
                                                                       str(window_bot),
                                                                       str(round(player.target.y))))
                    elif player.me.swing_ticks >= avg_swing:
                        player.state = PlayerState.PUCKMAN_STRIKE
                        print('STRIKE! dev: {4} f: top {0} bot {1} w: top {2} bot {3}'.format(
                            str(round(finish_puck_win_top)),
                            str(round(finish_puck_win_bot)),
                            str(round(window_top)),
                            str(round(window_bot)),
                ...
                if (player.state in PlayerState.PUCKMAN_STATES
                    ...
                    and const.puck_owner is not None
                    and const.puck_owner.id == player.me.id):
                    player.state = PlayerState.PUCKMAN_PANIC