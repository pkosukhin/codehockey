    ...
    for player in players:
        if player.state != PlayerState.REST:
            while not player.staged:
                player.staged = True
                ########################################################
                stam_mod = (0.75 + 0.25 * player.me.stamina/const.game.hockeyist_max_stamina)
                ...
                if (const.player.just_scored_goal or const.player.just_missed_goal):
                    if player.state not in(PlayerState.REST_FUNNY, PlayerState.GO_REST):
                        if const.player.just_scored_goal:
                            print('~~~   JUST SCORED GOAL!!!   ~~~')
                        else:
                            print('~~~   MISSED GOAL :(   ~~~')
                    player.state = PlayerState.REST_FUNNY
                if player.state in(PlayerState.GO_REST, PlayerState.REST_FUNNY) and \
                    ...
                    #print(player.me.id)
                    if player.me.teammate_index == nearest_teammate(const.game,const.world,const.def_x,const.def_y).teammate_index:
                ...
                #pre-states for turn+swing
                    ...
                    elif count_enemies_around(player.me.x, player.me.y, const.game.stick_length,const) >1:
                        for h in const.world.hockeyists:
                            if h.teammate and h.type != HockeyistType.GOALIE \
                                and h.state != HockeyistState.RESTING\
                                    and players[h.teammate_index].state != PlayerState.GO_REST:
                                if count_enemies_around(h.x, h.y, const.game.stick_length,const)==0:
                                    if abs(player.me.get_angle_to_unit(h)) < const.game.pass_sector/2*stam_mod*dex_mod*1.2\
            ...
            max_rest_str = 0
            for p in players:
                if p.me.state == HockeyistState.RESTING and p.me.stamina > max_rest:
                    max_rest = p.me.stamina
                    max_rest_str = p.me.strength
                if p.state == PlayerState.GO_REST:
                    cnt_rest +=1

            if ((player.me.stamina < const.game.hockeyist_max_stamina*(const.go_rest_pct+1 - min(1,player.me.strength/100))
                  or (const.player.just_scored_goal or const.player.just_missed_goal) and
                        player.me.stamina < const.game.hockeyist_max_stamina
                    or player.me.strength < 90 and (player.state != PlayerState.DEFMAN_GODEF_PERMANENT
                                                        or
                                                       const.puck_owner is not None and const.puck_owner.teammate) and
                        max_rest_str > 90)
                and player.state not in (PlayerState.PUCKMAN_STATES) and (cnt_rest == 0)
                    and max_rest> const.game.hockeyist_max_stamina*0.95):
                player.state = PlayerState.GO_REST
            player.staged = False
